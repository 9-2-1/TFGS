...
let blockText = ["下一个背景", "将.特效增加.", "将.特效设定为.", "清除图形特效", "背景.", "停止所有声音", "将.音效增加.", "将.音效设为.", "清除音效", "将音量增加.", "将音量设为.%", "当flag被点击", "当按下.键", "当.>.", "等待.秒", "重复执行.次.*", "重复执行.*", "如果.那么.", "如果.那么.否则.", "等待.", "重复执行直到..*", "询问.并等待", "回答", "按下.键?", "按下鼠标?", "鼠标的x坐标", "鼠标的y坐标", "响度", "计时器", "计时器归零", "当前时间的.", "2000年至今的天数", "用户名", ".+.", ".-.", ".*.", "./.", "在.和.之间取随机数", ".>.", ".<.", ".=.", ".与.", ".或.", ".不成立", "连接.和.", ".的第.个字符", ".的字符数", ".包含.?", ".除以.的余数", "四舍五入.", "..", "移动.步", "右转.度", "左转.度", "移到 x:.y:.", "在.秒内滑行到 x:.y:.", "面向.方向", "将x坐标增加.", "将x坐标设为.", "将y坐标增加.", "将y坐标设为.", "碰到边缘就反弹", "将旋转方式设为.", "x 坐标", "y 坐标", "方向", "说..秒", "说.", "思考..秒", "思考.", "下一个造型", "将大小增加.", "将大小设为.", "显示", "隐藏", "移到最.", "..层", "造型.", "大小", "音量", "当角色被点击", "当作为克隆体启动时", "删除此克隆体", "碰到颜色.?", "颜色.碰到.?", "将拖动模式设为.", "击打..拍", "休止.拍", "演奏音符..拍", "将乐器设为.", "将演奏速度设定为.", "将演奏速度增加.", "演奏速度", "全部擦除", "图章", "落笔", "抬笔", "将笔的颜色设为.", "将笔的.增加.", "将笔的.设为.", "将笔的粗细增加.", "将笔的粗细设为.", "当视频运动 >.", "相对于.的视频.", ".摄像头", "将视频透明度设为.", "朗读.", "使用.嗓音", "将朗读语言设置为.", "将.译为.", "访客语言", "移到.", "在.秒内滑行到.", "面向.", "换成.造型", "换成.背景", "播放声音.等待播完", "播放声音.", "当背景换成.", "当接收到.", "广播.", "广播.并等待", "停止.", "克隆.", "碰到.?", "到.的距离", ".的.", "将.设为.", "将.增加.", "显示变量.", "隐藏变量.", "将.加入.", "删除.的第.项", "删除.的全部项目", "在.的第.项前插入.", "将.的第.项替换为.", ".的第.项", ".中第一个.的编号", ".的项目数", ".包含.?", "显示列表.", "隐藏列表.", "当按下.键", "当依次按下.键时", "(.)", "\".\""];
...
let pinYin = get_pinyin_dict_notone();
let getPinYinCache = {};
// let matchCharCache = {};
...
function match(text) {
	...
	const CTYPE_BRACS = 3;
	// 测试子函数
	// return JSON.stringify(matchText.apply(this,text.split("|")));
	// return JSON.stringify(matchChar.apply(this,text.split("")));
	// return JSON.stringify(getPinYins(text));
	// return JSON.stringify(matchTextSimple.apply(this,text.split("|")));
	// 积木结构数据
	let blocksData = [{
			type: 0, // 初始文本积木
			parent: -1,
	...
	function guessPossibleMode() {
		...
		while (1) {
			...
			if (mode === 1) {
				...
				// i从1开始是为了跳过特殊的“.”模板
				for (let i = 1; i < blockType.length; i++) {
					let bi = blockType[i];
					if (bi.texts[0] !== "") {
						steps.push({
							text: bi.texts[0],
			...
			// debug(steps);
			// checkPossibleSteps;
			const SCORE_INPUT = 0.8;
			const PINYIN_DEC = 0.95;
			const TEXT_WEIGHT = 0.01;
			let choice = -1;
			let max = (mode === 1 && blocksData.length !== 1 ? SCORE_INPUT : 0),
				textIto;
			let textI0 = textI;
			...
			while (textI < text.length && choice === -1) {
				...
				for (let i = steps.length - 1; i >= 0; i--) {
					let si = steps[i];
					if (mode === 1 && !haveString && (si.stack !== -1 || si.mode !== 1)) {
						continue;
					}
					...
					let stext = si.text;
					// debug(i+"\t"+stext);
					let stextPs = getPinYins(stext);
					let subScore, subMax = 0,
						subTextIto;
					let subTextI = textI;
					while (textI < text.length) {
						...
						if (si.stack !== -1) {
							ftext = blocksData[blockStack[si.stack].blockId].text + "◆" + ftext;
						}
						let ftextPs = getPinYins(ftext);
						let match, subScore, subScoreP;
						match = matchText(ftext, stext);
						subScore = match.similarleft + stext.length * TEXT_WEIGHT;
						for (let x = 0; x < ftextPs.length; x++) {
							for (let y = 0; y < stextPs.length; y++) {
								match = matchText(ftextPs[x], stextPs[y]);
								subScoreP = (match.similarleft + stext.length * TEXT_WEIGHT) * PINYIN_DEC;
								if (subScoreP > subScore) {
									if (subScoreP > subMax) {
										debug(i + "\t" + stextPs[y] + "\t" + ftextPs[x] + "\t" + match.left + "\t" + subScoreP);
									}
									subScore = subScoreP;
								}
							}
			...
			if (choice === -1) {
				...
				textI = textIto;
				let matchWord = text.slice(textI1, textI);
				...
				if (step.stack !== -1) {
					...
					let frame = blockStack[step.stack];
					blockId = frame.blockId;
					inputId = frame.inputId;
					inputId++;
					blocksData[blockId].type = step.type;
					while (blocksData[blockId].inputs.length < blockType[step.type].inputs.length) {
						blocksData[blockId].inputs.push(-1)
					}
					blocksData[blockId].text += "◆" + matchWord;
					while (blockStack.length !== 0 && blockStack[blockStack.length - 1].stack >= frame.stack) {
						blockStack.splice(blockStack.length - 1, 1);
					}
				} else if (step.mode === 1) {
					textColor("red", "pink", textI0, textI1);
					let inputs = [];
					for (let i = 0; i < blockType[step.type].inputs.length; i++) {
						inputs.push(-1);
					}
					blocksData.push({
						type: step.type,
					...
					for (let i = 1; i < blockType[step.type].inputs.length; i++) {
						inputs.push(-1);
					}
				...
				let stackcnt = blockStack.length === 0 ? 0 : blockStack[blockStack.length - 1].stack + 1;
				let color = stackColor[stackcnt % stackColor.length];
				textColor("white", color, textI1, textI);

				//let haveInput = false;
				let stackId = blockStack.length === 0 ? 0 : blockStack[blockStack.length - 1].stack + 1;
				for (let i = 0; i < blockType.length; i++) {
					let bi = blockType[i];
					if (bi.inputs.length > inputId) {
						blockStack.push({
							blockId: blockId,
							inputId: inputId,
							stack: stackId,
							type: i,
							text: bi.texts.slice(0, inputId + 2).join("◆")
						});
						//haveInput = true;
					}
				}
	...
	function matchText(a, b) {
		...
		return {
			similarity: strSimilar / Math.max(a.length, b.length),
			similarleft: strSimilar / a.length,
		};
	}
	// 比对字符
	function matchChar(a, b) {
		const CASE_WEIGHT = 0.9;
		let similar = 0;
		if (a === b) {
			similar = 1;
	...
	// 获取可能的拼音(先不考虑多音字)
	function getPinYins(x) {
		let list;
		if (x in getPinYinCache) {
			return getPinYinCache[x];
		} else {
			list = [];
			getPinYinCh(0, "");
			getPinYinCache[x] = list;
			return list;
		}

		function getPinYinCh(i, str) {
			if (i === x.length) {
				list.push(str);
				return;
			}
			let match = false;
			for (let p in pinYin) {
				if (pinYin[p].includes(x[i])) {
					getPinYinCh(i + 1, str + p[0].toUpperCase() + p.slice(1));
					match = true;
					break;
				}
			}
			if (!match) {
				getPinYinCh(i + 1, str + x[i]);
			}
		}
	}
}
/** 来源: https://github.com/sxei/pinyinjs(使用MIT协议)
 * 收录常用汉字6763个，不支持声调，支持多音字，并按照汉字使用频率由低到高排序
 */
function get_pinyin_dict_notone() {
	return {
		"a": "阿啊呵腌嗄吖锕",
